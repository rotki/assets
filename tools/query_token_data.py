"""
Create SQL queries for asset updates using token data queried from Coingecko, Etherscan, and RPCs.
...
class Chain(Enum):
    ...
    SOLANA = -1  # negative id so it can be in the enum even though it doesn't have an evm chain id.
    ETHEREUM = 1
    BINANCE = 56
    BASE = 8453
    ARBITRUM_ONE = 42161
    ...
    def get_timestamp(self) -> None:
        while True:
            ...
            if deployed_time.isdigit():
                ...
                try:
                    # first maybe fix some things that confuse the time parser
                    if ' +UTC' in deployed_time:
                        deployed_time = deployed_time.replace(' +UTC', ' UTC')
                    elif ' (UTC)' in deployed_time:
                        deployed_time = deployed_time.replace(' (UTC)', ' UTC')
